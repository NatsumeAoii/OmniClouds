...
const { currentPath, breadcrumbs, searchTerm, isLoading } =
...
const view = useFileListView({
  ...
  loadFiles: () =>
    fileTreeStore
      .loadFiles(fileTreeStore.currentPath)
      .then(() => fileTreeStore.files),
  uploadQueueStore,
  autoRefresh: false,
  sortable: true,
...
function hasHighlightedFile(targetId) {
  return (
    Boolean(targetId) &&
    fileTreeStore.files.some((file) => file.id === targetId)
  );
}

function ensureHighlightedFileRendered(targetId) {
  const targetIndex = sortedFiles.value.findIndex(
...
async function handleUploads(entries) {
  ...
  try {
    await uploadQueueStore.uploadFiles(
      entries,
      currentPath.value,
...
async function readDirectoryEntry(entry, prefix = "") {
  ...
  const nested = await Promise.all(
    children.map((child) =>
      readDroppedEntry(child, prefix ? `${prefix}/${entry.name}` : entry.name),
    ),
  );
  return nested.flat();
}
      ...
      >
        <div>
          ...
          <p class="mt-2 text-sm text-[#5f6368] dark:text-slate-400">
            File dan folder akan diunggah ke lokasi Drive saat ini.
          </p>
        </div>
      </div>
